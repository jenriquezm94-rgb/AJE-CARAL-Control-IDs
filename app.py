...
def calcular_dias(fecha):
    if not fecha:
        return 0
    try:
        fe = datetime.strptime(fecha[:10], "%Y-%m-%d").date()
...
def get_registros():
    ...
    if buscar:
        ...
        datos = [r for r in datos if
                 ...
                 b in str(r.get("proveedor","")).lower()]
    return jsonify(datos)

@app.route("/api/registros", methods=["POST"])
def crear_registro():
    body = request.json
    compania_code = "AJE" if "AJEPER" in body.get("compania","").upper() else "CAR"
    today = date.today()
    prefijo = f"{compania_code}-{today.strftime('%Y%m')}"
    res = supabase.table("registro").select("numero_id").like("numero_id", f"{prefijo}%").execute()
    siguiente = len(res.data or []) + 1
    body["numero_id"] = f"{prefijo}-{str(siguiente).zfill(3)}"
    body["fecha_emision"] = body.get("fecha_emision") or today.isoformat()
    body["estado_oc"] = body.get("estado_oc") or "PENDIENTE"
    body["dias_sin_oc"] = 0
...
def actualizar_registro(id):
    ...
    if body.get("estado_oc") in ("OC CONFIRMADA", "OC RECIBIDA"):
        body["dias_sin_oc"] = 0
    res = supabase.table("registro").update(body).eq("id", id).execute()
...
def get_materiales():
    ...
    query = supabase.table("material").select("*")
    if buscar:
        query = query.ilike("descripcion", f"%{buscar}%")
    datos = query.limit(20).execute().data or []
    return jsonify(datos)
...
def get_dashboard():
    ...
    total_mes = sum(1 for r in datos if str(r.get("fecha_emision","")).startswith(mes_actual))
    pendientes = [r for r in datos if r.get("estado_oc") == "PENDIENTE"]
    for r in pendientes:
        r["dias_sin_oc"] = calcular_dias(r.get("fecha_emision"))
    ...
    confirmadas= sum(1 for r in datos if r.get("estado_oc") == "OC CONFIRMADA")
    alertas    = [r for r in pendientes if r["dias_sin_oc"] >= 4]
    return jsonify({
        "total_mes": total_mes,
        "vencidos": vencidos,
        "en_limite": en_limite,
